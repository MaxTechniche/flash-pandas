            ...
            [
                ...
                dbc.Row(
                    [
                        ...
                        dbc.Label(
                            id="report-code", style={"margin": "7px", "color": "red"}
                        ),
                        dbc.Row(
                            [
                                dbc.Input(
            ...
            [
                    ...
                    [
                        dbc.Col(
                            [
                                ...
                                dcc.Markdown(
                                    card["question_text"], id="question", style=qa_style
                                ),
                            ],
                        ...
                        dbc.Col(
                            [
                                ...
                                dcc.Markdown(
                                    card["answer_text"], id="answer", style=qa_style
                                ),
                            ],
                            style={"min-width": "200px"},
                        ),
                ...
                dbc.Row(
                    [
                        ...
                        dbc.Label("Creator: " + card["creator"], className="ml-auto"),
                    ],
                    style={"margin": "auto"},
                ),
                html.Div(
                    [
                        ...
                        dbc.Label(
                            "Card ID: " + str(card["_id"]), style={"font-size": "12px"}
                        ),
                    ],
...
@APP.callback(
    ...
    [State("card-id", "value"), State("flag-reason", "value")],
)
def report_card(n_clicks, card_id, reason):
    if n_clicks:
        ...
        try:
            ...
            return "Invalid ID"
        code = cards.update_one(
            {"_id": obj_id},
            {
                "$set": {
                    "flagged": True,
                    "flag_reason": reason or "Unknown",
                    "public": False,
                }
            },
        )
...
@APP.callback(Output("question", "style"), Input("show-questions", "checked"))
def show_questions(show):
    if show:
        next = qa_style.copy()
        next.update({"filter": "blur(0)"})
        return next
    return qa_style


@APP.callback(Output("answer", "style"), Input("show-answers", "checked"))
