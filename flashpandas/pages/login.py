...
@APP.callback(Output("url", "pathname"), Input("log-out", "n_clicks"))
...
@APP.callback(Output("password-entry", "type"), Input("pass-toggle", "checked"))
...
@APP.callback(
    ...
    [State("username-entry", "value"), State("password-entry", "value")],
)
def check_login(login_click, username, password):
    if login_click:
        ...
        user_info = users.find_one({"username": username}) or users.find_one(
            {"email": username}
        )
